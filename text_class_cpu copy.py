...
'''
 Author       : Xuexin
 Date         : 2023-09-30 21:25:13
 LastEditTime : 2023-10-07 22:30:43
 FilePath     : \\torch\\text_class.py
 Description  : 文本分类项目训练与测试
'''
...
class GCNN(nn.Module):
    def __init__(self, vocab_size=VOCAB_SIZE, embedding_dim=64, num_class=2):
        ...
        self.embedding_tabel = nn.Embedding(vocab_size, embedding_dim)
        nn.init.xavier_uniform_(self.embedding_tabel.weight) # 参数初始化为均匀分布
    ...
    def forward(self, word_index):
        # 定义GCN的算子操作流程， 基于句子单词ID输入的得到分类logits输出

        # 1. 通过word_index得到word_embedding
        # word_index shape:[bs, max_seq_len]

        word_embedding = self.embedding_tabel(word_index) #[bs, max_seq_len, embedding_dim]
        # 2. 编写第一层1D门卷积模块
...
class TextClassificationModel(nn.Module):
    """简单版的embeddingbag+DNN模型"""

    def __init__(self, vocab_size=VOCAB_SIZE, embed_dim=64, num_class=2):
        super(TextClassificationModel, self).__init__()
        self.embedding = nn.EmbeddingBag(vocab_size, embed_dim, sparse=False) # sparse 稀疏的
...
train_data_iter = IMDB(root='.data', split='train') # Dataset类型对象
tokenizer = get_tokenizer("basic_english")
vocab = build_vocab_from_iterator(yield_tokens(train_data_iter, tokenizer), min_freq=20, specials=["<unk>"])
vocab.set_default_index(0) # 如果找不到单词设置默认索引,后续可用于padding
...
def collate_fn(batch):
    ...
    token_index = [index + [0]*(max_length-len(index)) for index in token_index] # padding到batch最长
    return (torch.tensor(target).to(torch.int64), torch.tensor(token_index).to(torch.int32))


# step3 编写训练代码
def train(train_data_loader, eval_data_loader, model, optimizer, num_epoch, log_step_interval, save_step_interal,\
           eval_step_interval, save_path, resume=''):
    """此处的data_loader是map-style dataset"""
    start_epoch = 0
    start_step = 0
    if resume != '':
        # 加载之前训练过的模型的参数文件
    ...
    # print(f"start_epoch是{start_epoch}")
    for epoch_index in range(start_epoch, num_epoch):
        ...
        for batch_index, (target, token_index) in enumerate(train_data_loader):
            ...
            bce_loss = F.binary_cross_entropy(torch.sigmoid(logits), F.one_hot(target, num_classes=2).to(torch.float32)) # 计算loss需要将one_hot转换为float32
            ema_loss = 0.9*ema_loss + 0.1*bce_loss # 用于观测，移动平均更加稳定
            bce_loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 0.1) # 对梯度的模进行截断，让训练更加稳定
            optimizer.step()

            if step % log_step_interval == 0:
                logging.warning(f"epoch_index:{epoch_index}, batch_index:{batch_index}, ema_loss:{ema_loss.item()}")
            if step % save_step_interal == 0:
...
if __name__ == '__main__':
    ...
    eval_data_iter = IMDB(root='.data', split='test') # Dataset类型对象
    eval_data_loader = torch.utils.data.DataLoader(to_map_style_dataset(eval_data_iter), batch_size = BATCH_SIZE,\
                                                    collate_fn=collate_fn, shuffle=True)
    # resume = ''
    resume = r'D:\learn\torch\logs_imdb_text_classification\step_500.pt'
    train(train_data_loader, eval_data_loader, model, optimizer, num_epoch=10, log_step_interval=20, save_step_interal=500,\
